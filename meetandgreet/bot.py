...
from logic.signals import feedback_data_ready
from logic.models import Feedback
...
# Add handler for delete manager
def add_manager(update: Updater, context: CallbackContext):
    if check_white_list(update.effective_chat.id) and update.message.text == 'add_manager':
        try:
